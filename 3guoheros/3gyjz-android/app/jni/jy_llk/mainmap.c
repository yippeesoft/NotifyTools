	...
	int i,j;
		...
		JY_FillColor(0,0,0,0,0);
	 	...
	 	if(px >= 0 && py >= 0){
	 		...
	 		struct CacheNode* cache = GetPicCache(0,Mypic*2);

	 		rect.x = px-cache->xoff;
			rect.y = py-cache->yoff;
			rect.w = cache->w;
			rect.h = cache->h;

	 		tex = SDL_CreateTexture(g_renderer,SDL_PIXELFORMAT_ARGB8888,SDL_TEXTUREACCESS_TARGET,cache->w,cache->h);
	 		SDL_SetRenderTarget(g_renderer, tex);
			SDL_RenderClear(g_renderer);
			SDL_RenderCopy(g_renderer, cache->s, NULL, NULL);
	 		...
	 		*/
	 		...
	 		SDL_Rect rect;
	 		struct CacheNode* cache = GetPicCache(0,Mypic*2);
	 		SDL_SetTextureAlphaMod(cache->s,96);
	 		rect.x = px-cache->xoff;
			rect.y = py-cache->yoff;
			rect.w = cache->w;
			rect.h = cache->h;
	 		SDL_RenderCopy(g_renderer, cache->s,NULL,  &rect);

	 		SDL_SetTextureAlphaMod(cache->s,255);
	 	}
		    ...
		    for(i=istart;i<=iend;i++){
				...
				if( (xx>=0) && (xx<War_XMax) && (yy>=0) && (yy<War_YMax) ){
					...
					if(n > 0)		//绘制吉凶地
					{
						...
						if(tmpTex[0] == 0)
						{
							tmpTex[0] = createPolygonTexture(0x05,0xd0,0x10,255);
							tmpTex[1] = createPolygonTexture(0xd5,0x22,0x10,255);
							tmpTex[2] = createPolygonTexture(0,0,0xf0,255);
							tmpTex[3] = createPolygonTexture(0xa0,0x10,0xa0,255);
						}
		    ...
		    for(i=istart;i<=iend;i++){
				...
				if( (xx>=0) && (xx<War_XMax) && (yy>=0) && (yy<War_YMax) ){
					if(JY_GetWarMap(xx,yy,3)<128){
						...
						if(flag==1)
						{
							if((xx==v1)&&(yy==v2))	tex = g_WarMoveTex[2];
							else tex = g_WarMoveTex[3];
						}
						else
						{
							if((xx==v1)&&(yy==v2))	tex = g_WarMoveTex[0];
							else tex = g_WarMoveTex[1];
						}
